...
<template>
  <div class="archive">
    ...
    <details v-for="f in favList" :key="f.id" class="fav">
      <summary>
    ...
    <!-- 近期正文档案 -->
    <div class="ar-sec">近期正文档案（系统暂存·最近3天·超期自动清理）</div>
    <p class="ar-hint">这里是引擎为前情注入暂存的原文（每段截断保留末段）。遗漏未收藏的段落可以从这里补救——注意补救版是截断的，完整版只在结算当时可收。</p>
    ...
    <details v-for="p in archList" :key="p.id" class="fav arch">
      <summary>
        ...
        <button class="f-del save" @click.prevent.stop="onSaveArch(p)" :disabled="savedIds.has(p.id)">{{ savedIds.has(p.id) ? '✓ 已收藏' : '❤ 收藏(截断版)' }}</button>
      </summary>
...
const favList = computed(() => [...r.favorites].reverse());
...
function onSaveArch(p: ProseEntry) {
  const ok = r.addFavorite({ id: `arch-${p.id}`, day: p.day, label: `${p.label}（截断版）`, text: p.text });
